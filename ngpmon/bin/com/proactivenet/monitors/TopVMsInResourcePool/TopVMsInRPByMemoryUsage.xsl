...
<xsl:stylesheet
   ...
   <body>
   		    <table class="section-header" width="100%" border="0" cellspacing="0" cellpadding="0">
                           <tr>
                               <td class="caption">
                               Top 10 VMs by Memory Usage on 
			           <xsl:for-each select="detaildiagnostics/data/top/resourcepool">
			               <xsl:value-of select="resourcepoolname"/> 
        		           </xsl:for-each>
						   on VC/ESX: <xsl:for-each select="detaildiagnostics/data/top/vcesx">
						   <xsl:value-of select="vcesxname"/> 
						  </xsl:for-each>
                                </td>
                               <td align="right">
                                   <a target="_new" href="/servlets/com.proactivenet.servlet.HelpServlet?title=TopVMsInRPByMemoryUsage">
    ...
    <xsl:otherwise>
      <table class="data" width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr class="header-column first">
          ...
          <th class="header-column third" style="text-align: center;">Memory Usage(%)</th>
        </tr>
